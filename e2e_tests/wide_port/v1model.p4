// Modified copy of p4c's v1model.p4 with PortId_t widened from bit<9> to
// bit<16>.  Used to test that the simulator correctly derives port-field widths
// from the compiled IR rather than hardcoding them.  See Track 5 in ROADMAP.md.
//
// The only change from the upstream file is on the PortId_t typedef line.
...
typedef bit<16> PortId_t;       // should not be a constant size?
#endif
