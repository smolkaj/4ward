 ...
 *
 * Different P4 architectures (v1model, PSA, PNA) share the same IR interpreter
 * but differ in:
 *
 ...
 *
 * Points 1 and 2 are driven by the [fourward.ir.v1.Architecture] proto in the
 * loaded pipeline config. Point 3 requires code here.
 *
 * To add a new architecture, implement this interface and register the
 ...
 *
 * [outputPackets] are the packets to emit (port, payload).
 * [trace] is the combined execution trace across all pipeline stages.
 */
data class PipelineResult(
    val outputPackets: List<OutputPacket>,
    val trace: fourward.sim.v1.Trace,
)
