 ...
 *
 * All arithmetic is performed with BigInteger and then masked to [width] bits, matching
 * the P4 spec's truncating-on-overflow semantics. No operation ever produces a value
 * outside [0, 2^width).
 */
data class BitVector(val value: BigInteger, val width: Int) {
    ...
    fun subSat(other: BitVector): BitVector {
        requireSameWidth(other)
        val result = value - other.value
        return if (result < BigInteger.ZERO) BitVector(BigInteger.ZERO, width) else BitVector(result, width)
    }
    ...
    fun slice(hi: Int, lo: Int): BitVector {
        ...
        return BitVector((value shr lo) and BigInteger.TWO.pow(newWidth).minus(BigInteger.ONE), newWidth)
    }
    ...
    private val mask: BigInteger get() = BigInteger.TWO.pow(width).minus(BigInteger.ONE)
    private val max: BigInteger  get() = mask
    ...
    companion object {
        ...
        fun ofLong(value: Long, width: Int): BitVector =
            BitVector(BigInteger.valueOf(value), width)
 ...
 *
 * Stored internally as a two's-complement signed BigInteger in the range
 * [-2^(width-1), 2^(width-1)).
 */
