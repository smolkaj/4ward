 ...
 *
 * Reads [SimRequest] messages from stdin and writes [SimResponse] messages to
 * stdout, using length-delimited framing: each message is preceded by a 4-byte
 * big-endian integer giving the number of bytes in the serialised message body.
 *
 * This is the same framing used by Bazel's persistent worker protocol and the
 * Language Server Protocol. It is simple, reliable, and well-supported in both
 * Kotlin and Go.
 *
 * Stderr is reserved for diagnostic output (startup messages, unexpected errors).
 * Never write SimResponse bytes to stderr or diagnostic text to stdout.
 */
fun main() {
 ...
 *
 * Returns null on EOF (the controller closed the pipe — normal shutdown).
 * Throws on IO errors or malformed messages.
 */
 ...
 *
 * Flushes immediately so the controller receives the response without waiting
 * for the output buffer to fill.
 */
