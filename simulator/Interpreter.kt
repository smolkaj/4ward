 ...
 *
 * Walks the proto IR tree for a single packet traversal. All mutable
 * packet-local state lives in the [Environment]; program-global state
 * (table entries, extern instances) lives in [TableStore] and is passed in.
 *
 * The interpreter is deliberately simple: it pattern-matches on proto oneof
 * fields and dispatches to focused methods. There is no bytecode compilation
 * or optimisation — correctness and readability are the goals.
 */
class Interpreter(
    private val config: P4BehavioralConfig,
    private val tableStore: TableStore,
) {
    private val parsers: Map<String, ParserDecl> =
        config.parsersList.associateBy { it.name }

    private val controls: Map<String, ControlDecl> =
        config.controlsList.associateBy { it.name }

    private val actions: Map<String, fourward.ir.v1.ActionDecl> =
        config.actionsList.associateBy { it.name }

    private val tables: Map<String, TableBehavior> =
        config.tablesList.associateBy { it.name }
    ...
    private fun runParserState(parser: ParserDecl, stateName: String, env: Environment) {
        ...
        val state = parser.statesList.find { it.name == stateName }
    ...
    private fun execIf(ifStmt: fourward.ir.v1.IfStmt, env: Environment) {
        ...
        env.addTraceEvent(
            TraceEvent.newBuilder()
                .setBranch(BranchEvent.newBuilder().setTaken(condition))
                .build()
        )
    ...
    fun evalExpr(expr: Expr, env: Environment): Value = when {
        expr.hasLiteral()     -> evalLiteral(expr.literal, expr.type)
        expr.hasNameRef()     -> env.lookup(expr.nameRef.name)
            ?: error("undefined variable: ${expr.nameRef.name}")
    ...
    private fun evalArrayIndex(ai: fourward.ir.v1.ArrayIndex, env: Environment): Value {
        val stack = evalExpr(ai.expr, env) as? HeaderStackVal
            ?: error("array index on non-stack value")
    ...
    private fun applyTable(tableName: String, env: Environment): TableResult {
        ...
        val keyValues = tableBehavior.keysList.map { key ->
            key.fieldName to evalExpr(key.expr, env)
        }
    ...
    private fun execAction(
        ...
        try {
            actionDecl.paramsList.forEachIndexed { i, paramDecl ->
                ...
                val value: Value = if (paramProto != null) {
