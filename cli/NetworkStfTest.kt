...
class NetworkStfTest {
  ...
  @Test
  fun `switch without stf path is allowed`() {
    val nstf = parseNstf("switch s1 pipeline.txtpb")
    assertEquals(1, nstf.switches.size)
    assertEquals(null, nstf.switches[0].stfPath)
  }

  @Test
  fun `parses inline table entries`() {
    val nstf =
      parseNstf(
        """
      switch s1 pipeline.txtpb
      s1: add port_table hdr.ethernet.etherType:0x0800 forward(1)
      s1: setdefault port_table drop()
      """
      )
    assertEquals(2, nstf.inlineEntries["s1"]?.size)
    assertEquals(
      "add port_table hdr.ethernet.etherType:0x0800 forward(1)",
      nstf.inlineEntries["s1"]!![0],
    )
    assertEquals("setdefault port_table drop()", nstf.inlineEntries["s1"]!![1])
  }

  @Test
  fun `rejects undeclared switch in inline entries`() {
    val e =
      assertThrows(IllegalArgumentException::class.java) {
        parseNstf("switch s1 pipeline.txtpb\ns2: add table foo bar()")
      }
    assertEquals("inline entries reference undeclared switch 's2'", e.message)
  }
